...
def init_db():
    ...
    users = [
        ...
        (2,"alice","password123","user","alice@workshop.lab",f"Personal: {FLAGS['auth_3']}",1000),
        (3,"bob","bobsecure!","user","bob@workshop.lab","I like cats.",1000),
        (4,"charlie","charlie456","user","charlie@workshop.lab","Nothing here.",1000),
    ]
    for u in users:
...
# ═══════════════════════════════════════════════════════════════
# LAB 2: CSRF (2 flags) — requires request from outside
# ═══════════════════════════════════════════════════════════════
@app.route("/csrf")
def csrf_hub():
        ...
        if to_user == "attacker" and amount >= 500:
            if "csrf/bank" in referer:
                return render_template("csrf_bank.html",
                    msg=f"Transferred Nu.{amount} to {to_user}. But this was a normal transfer — "
                        f"you submitted the form yourself. The flag requires the request to come "
                        f"from OUTSIDE this page. That's what makes it a CSRF attack.")
            return render_template("csrf_success.html", flag=FLAGS["csrf_1"], amount=amount)
        return render_template("csrf_bank.html", msg=f"Transferred Nu.{amount} to {to_user}")
...
def csrf_bank2():
    ...
    if request.method == "POST":
        ...
        # BUG: checks existence not value
        to_user = request.form.get("to","")
        amount = int(request.form.get("amount", 0))
        referer = request.headers.get("Referer", "")
        if to_user == "attacker" and amount >= 500:
            if "csrf/bank2" in referer:
                return render_template("csrf_bank2.html",
                    msg=f"Transferred Nu.{amount} to {to_user}. Normal transfer — "
                        f"the flag requires the request from outside. "
                        f"Also — look at how the token is validated. Is the VALUE checked?",
                    token=session["csrf_token"], error=None)
            return render_template("csrf_success.html", flag=FLAGS["csrf_2"], amount=amount)
        return render_template("csrf_bank2.html", msg=f"Transferred Nu.{amount} to {to_user}", token=session["csrf_token"], error=None)
    return render_template("csrf_bank2.html", msg=None, token=session["csrf_token"], error=None)

# ═══════════════════════════════════════════════════════════════
# LAB 3: AUTH BYPASS (5 flags)
# ═══════════════════════════════════════════════════════════════
@app.route("/auth")
def auth_hub():
...
def auth_l1():
    ...
    if request.method == "POST":
        ...
        if user and user["role"] == "admin":
            return render_template("auth_success.html", flag=FLAGS["auth_1"],
                                   msg=f"Logged in as {user['username']} ({user['role']})")
...
def auth_2_admin():
    ...
    try:
        ...
        return jsonify({"error":"Insufficient privileges","role":payload.get("role")}), 403
...
def auth_4_dash():
    ...
    if not token: return jsonify({"error":"No session token provided"}), 401
    db = get_db()
    ...
    if not sess: return jsonify({"error":"Invalid or expired session"}), 401
    result = dict(sess)
...
def auth_5_panel():
    ...
    try:
        ...
        return jsonify({"error":"Admin role required","your_claims":payload}), 403
    except Exception as e:
        return jsonify({"error":str(e)}), 401
...
def api_docs():
    return jsonify({
        ...
        "note": "This is the complete API documentation."
    })
